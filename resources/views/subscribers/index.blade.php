...
@section('content')
    ...
    @component('marketing::layouts.partials.actions')
        ...
        @slot('left')
            <form action="{{ route('marketing.subscribers.index') }}" method="GET" class="form-inline mb-3 mb-md-0">
                ...
                @if(request()->anyFilled(['name', 'status']))
                    <a href="{{ route('marketing.subscribers.index') }}"
        ...
        @slot('right')
            <div class="btn-group mr-2">
                ...
                <div class="dropdown-menu">
                    <a href="{{ route('marketing.subscribers.import') }}" class="dropdown-item">
                        <i class="fa fa-upload color-gray-400 mr-2"></i> {{ __('Import Subscribers') }}
                    </a>
                    <a href="{{ route('marketing.subscribers.export') }}" class="dropdown-item">
                        <i class="fa fa-download color-gray-400 mr-2"></i> {{ __('Export Subscribers') }}
                    </a>

                </div>
            </div>
            <a class="btn btn-light btn-md mr-2" href="{{ route('marketing.tags.index') }}">
                <i class="fa fa-tag color-gray-400 mr-1"></i> {{ __('Tags') }}
            </a>
            <a class="btn btn-primary btn-md btn-flat" href="{{ route('marketing.subscribers.create') }}">
            ...
            <table class="table">
                ...
                @forelse($subscribers as $subscriber)
                    <tr>
                        ...
                        <td>
                            <form action="{{ route('marketing.subscribers.destroy', $subscriber->id) }}" method="POST">
                                @csrf
                                @method('DELETE')
                                <a href="{{ route('marketing.subscribers.edit', $subscriber->id) }}"
    ...
    @include('marketing::layouts.partials.pagination', ['records' => $subscribers])
