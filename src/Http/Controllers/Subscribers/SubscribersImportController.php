...
use Cornatul\Marketing\Base\Http\Requests\SubscribersImportRequest;
use Cornatul\Marketing\Base\Repositories\TagTenantRepository;
use Cornatul\Marketing\Base\Services\Subscribers\ImportSubscriberService;
...
class SubscribersImportController extends Controller
{
    ...
    {
        $tags = $tagRepo->pluck(MarketingPortal::currentWorkspaceId(), 'name', 'id');

        return view('marketing::subscribers.import', compact('tags'));
    }
    ...
    public function store(SubscribersImportRequest $request): RedirectResponse
    {
        if ($request->file('file')->isValid()) {
            ...
            (new FastExcel)->import(Storage::disk('local')->path($path), function (array $line) use ($request, &$counter) {
                ...
                $subscriber = $this->subscriberService->import(MarketingPortal::currentWorkspaceId(), $data);
