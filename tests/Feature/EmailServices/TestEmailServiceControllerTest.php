...
class TestEmailServiceControllerTest extends TestCase
{
    ...
    {
        // given
        $service = EmailService::factory()->create(['workspace_id' => MarketingPortal::currentWorkspaceId()]);

        // when
        $response = $this->get(route('marketing.email_services.test.create', $service->id));
    ...
    public function an_email_service_cannot_be_tested_without_a_from_email_address()
    {
        ...
        // when
        $response = $this->from(route('marketing.email_services.test.create', $emailService->id))
            ->post(route('marketing.email_services.test.store', $emailService->id), $postData);

        // then
        $response->assertRedirect(route('marketing.email_services.test.create', $emailService->id));
    ...
    public function an_email_service_can_be_tested_by_an_authenticated_user()
    {
        ...
        $this->instance(DispatchTestMessage::class, Mockery::mock(DispatchTestMessage::class, function ($mock) use ($emailService, $from) {
            $mock->shouldReceive('testService')
                ...
                ->withArgs(function ($workspaceId, $targetService, MessageOptions $options) use ($emailService, $from) {
                    return $workspaceId === MarketingPortal::currentWorkspaceId()
        ...
        // then
        $response->assertRedirect(route('marketing.email_services.index'));
